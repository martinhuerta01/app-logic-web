...
const TIPOS       = ["-", "INSTALACION", "REVISION", "DESINSTALACION"];
const DISPOSITIVOS = ["-", "GPS", "LECTORA", "GPS y LECTORA", "CAMARA", "Tractor", "Semi", "Chasis"];
const ESTADOS     = ["-", "PENDIENTE", "CONFIRMADO", "REALIZADO", "SUSPENDIDO"];
...
export default function CargaDiaPage() {
  ...
  // Duplicar la fila anterior al presionar Enter en Patente
  const onPatenteKeyDown = (e, fila) => {
    if (e.key === "Enter") {
      e.preventDefault();
      const nueva = { ...filaVacia(), tipo: fila.tipo, dispositivo: fila.dispositivo, estado: fila.estado };
      setFilas(prev => {
  ...
  const toggleFeriado = (checked) => {
    ...
    if (checked) {
      setSvcCliente("-");
      setFilas([{ ...filaVacia(), tipo: "-", dispositivo: "-", estado: "-" }]);
    } else {
      setSvcCliente("");
      setFilas([filaVacia()]);
    }
  ...
  const guardarBloque = async (e) => {
    ...
    const filasValidas = feriado
      ? filas.slice(0, 1)
      : filas.filter(f => f.patente.trim());
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={guardarBloque} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className={esInterior ? "" : "md:col-span-1"}>
        ...
        <div>
          ...
          <div className="border border-slate-200 rounded-lg overflow-hidden">
            {/* Cabecera tabla */}
            <div className="grid bg-slate-50 border-b border-slate-200 text-xs font-semibold text-slate-500 px-3 py-2"
              style={{gridTemplateColumns: "160px 160px 120px 150px 1fr 36px"}}>
              <span>Tipo</span>
            ...
            <div className="divide-y divide-slate-100">
              {filas.map((fila, idx) => (
                <div key={fila._id}
                  className="grid items-center gap-2 px-3 py-2 hover:bg-slate-50"
                  style={{gridTemplateColumns: "160px 160px 120px 150px 1fr 36px"}}>
                  ...
                  <button type="button"
                    ...
                    className="text-red-400 hover:text-red-600 transition disabled:opacity-20 disabled:cursor-not-allowed flex justify-center">
                    <IconTrash />
                  </button>
                </div>
              ))}
        ...
        <div className="flex items-center gap-4 pt-1">
          <button type="submit" disabled={guardando}
            ...
            {guardando ? "Guardando…" : `Guardar ${filas.filter(f => f.patente.trim() || feriado).length || ""} servicio${filas.filter(f => f.patente.trim()).length !== 1 ? "s" : ""}`}
          </button>

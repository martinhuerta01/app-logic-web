...
export default function HistorialCamionetaPage() {
  ...
  const esEquipo2 = (m) => (m.equipos?.nombre || equipos.find(e => String(e.id) === String(m.equipo_id))?.nombre) === "Equipo 2";
  const equipoEditNombre = equipos.find(e => String(e.id) === String(editando?.equipo_id))?.nombre;
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 text-slate-600 bg-slate-50 text-xs">
              ...
              <th className="text-left px-4 py-3">Llegada GR/LCH</th>
              <th className="text-left px-4 py-3">Salida GR/LCH</th>
          ...
          <tbody>
            {movimientos.length === 0 ? (
              <tr><td colSpan={10} className="px-4 py-4 text-slate-400 text-xs">Sin movimientos</td></tr>
            ) : movimientos.map(m => (
              <tr key={m.id} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="px-4 py-2.5 text-xs">{esEquipo2(m) ? (m.llegada_gr_lch?.slice(0,5) || "—") : <span className="text-slate-300">—</span>}</td>
                <td className="px-4 py-2.5 text-xs">{esEquipo2(m) ? (m.salida_gr_lch?.slice(0,5) || "—") : <span className="text-slate-300">—</span>}</td>
                <td className="px-4 py-2.5 text-xs">{m.cargado_por || "—"}</td>
                <td className="px-4 py-2.5 text-xs">
                  <div className="flex gap-2">
                    <button onClick={() => setEditando({ ...m, equipo_id: m.equipo_id || m.equipos?.id || "" })}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={guardarEdicion} className="bg-white rounded-xl shadow-xl border border-slate-200 p-6 w-full max-w-lg space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {equipoEditNombre === "Equipo 2" && (
                <>
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">Llegada GR/LCH</label>
                    <input type="time" value={editando.llegada_gr_lch || ""} onChange={e => setEditando({ ...editando, llegada_gr_lch: e.target.value })}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs text-slate-500 mb-1">Salida GR/LCH</label>
                    <input type="time" value={editando.salida_gr_lch || ""} onChange={e => setEditando({ ...editando, salida_gr_lch: e.target.value })}
                      className="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
                  </div>
                </>
              )}
